...
(async function main(){
  ...
  if (!b64) {
    console.log("ERROR: FIREBASE_SERVICE_ACCOUNT_BASE64 missing. Cannot reset Firestore content.");
    process.exit(1);
  }
  ...
  await db.collection("siteContent").doc("home").set({
    ...
    trustBody: "We are registered. Your money is safe with us. When you need support, we can assist you with loan or overdraft based on your savings.",
    updatedAt: now
  }, { merge: true });

  await db.collection("siteContent").doc("about").set({
    title: "About Us",
    body: "Godsfavour Blossom is a cooperative society built to support people through savings, friendly loan, overdraft and cooperative services.\n\nEstablished: September 2012.\n\nOur goal is simple: your money is safe with us, and we support you when you need it.",
    trustTitle: "Registered & Trusted",
    trustBody: "We are registered, and we run our cooperative with trust and responsibility.",
    adminImageUrl: "/admin.jpg",
    cacImageUrl: "/cac.jpg",
    bePartTitle: "Be a part of us",
    bePartBody: "If you want to join as a member or shareholder, fill the form and submit. We will contact you with the next steps.",
    updatedAt: now
  }, { merge: true });

  // Services — overwrite items by slug to eliminate old 5% remnants
  const items = [
    {
      slug: "savings",
      title: "Savings",
  ...
  await db.collection("siteContent").doc("services").set({
    title: "Services",
  ...
  console.log("DONE: Firestore content reset to new admin rules.");
